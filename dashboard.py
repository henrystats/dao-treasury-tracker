...
# ───── static logos / colours (trim these as you like) ─────
TOKEN_LOGOS = {
...
def token_category(tok:str)->str:
    ...
    if any(s in T for s in ("USDC","USDT","DAI","USDE")): return "Stables"
    return "Others"
...
def load_history():
    try:
        ...
        return pd.DataFrame(columns=["timestamp","history_type","name","usd_value"])
...
if not week.empty:
    ...
    if not p.empty:
        ...
        chartA.plotly_chart(
            px.area(p, x="timestamp", y="usd_value", color="name",
    ...
    if not t.empty:
        ...
        chartB.plotly_chart(
            px.area(t, x="timestamp", y="usd_value", color="name",
    ...
    for proto in order.index:
        ...
        for cls in sub["Classification"].dropna().unique():
            ...
            part["Token"] = part["Token"].apply(lambda t:
                f'<img src="{TOKEN_LOGOS.get(t,"")}" width="16" style="vertical-align:middle;margin-right:4px;"> {t}')
            st.markdown(md_table(part[["Wallet","Token","USD Value"]],
