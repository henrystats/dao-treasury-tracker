...
df_wallets = pd.DataFrame(wallet_rows).copy()
...
df_protocols = pd.DataFrame(prot_rows).copy()
...
if not df_protocols.empty:
    ...
    for proto in order.index:
        st.markdown(f'<h3><img src="{PROTOCOL_LOGOS.get(proto,"")}" width="24" style="vertical-align:middle;margin-right:6px;">'
